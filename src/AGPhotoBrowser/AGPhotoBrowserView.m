...
- (void)configureCell:(UITableViewCell<AGPhotoBrowserCellProtocol> *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_dataSource respondsToSelector:@selector(photoBrowser:URLStringForImageAtIndex:)] && [cell respondsToSelector:@selector(setCellImageWithURL:)]) {
        ...
        NSString* imageUrl = [_dataSource photoBrowser:self imageAtIndex:indexPath.row];

        SDWebImageManager *manager = [SDWebImageManager sharedManager];
        [manager downloadImageWithURL:[NSURL URLWithString:imageUrl] options:0
...
- (void)didTapOnZoomableViewForCell:(id<AGPhotoBrowserCellProtocol>)cell withRecognizer:(UITapGestureRecognizer *)recognizer
{
