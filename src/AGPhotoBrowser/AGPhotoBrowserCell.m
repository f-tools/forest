...
- (AGPhotoBrowserZoomableView *)zoomableView
{
    if (!_zoomableView) {
        ...
        [_zoomableView addGestureRecognizer:tap];
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if (gestureRecognizer == self.panGesture) {
        ...
        if (fabsf((float)translation.x) > fabsf((float)translation.y)) {
            return YES && self.zoomableView.canPan;
            ;
        }
    }
...
{
    //  if ([self.zoomableView canPan]) {
    [self.delegate didPanOnZoomableViewForCell:self withRecognizer:recognizer];
    //}
}
- (void)doubleTapped:(UITapGestureRecognizer *)recognizer
{
    [self.delegate didTapOnZoomableViewForCell:self withRecognizer:recognizer];
}
